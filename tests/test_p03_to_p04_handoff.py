...
def test_current_state_is_p03_closed_and_ready_for_p04():
 d=json.loads((ROOT/'CURRENT_PROJECT_STATUS.json').read_text())
 assert d['completed_phase_scopes']==['P00','P01','P02','P03']
 assert d['p02_execution_complete'] is True
 assert d['p03_execution_started'] is True and d['p03_execution_complete'] is True
 assert d['current_phase_closed']=='P03'
 assert d['scientific_blockers']==d['documentary_blockers']==d['major_issues']==0

def test_l1_l2_l3_root_import_boundaries_are_nonexecuting():
...
def test_p03_to_p04_readiness_is_explicit_and_unblocked():
 h=yaml.safe_load((ROOT/'phase_handoff.yaml').read_text())['phase_handoff']
 assert h['consumer_phase']=='P04' and h['producer_phase']=='P03' and h['blockers']==[]
 assert h['status']=='READY_WITH_EXPLICIT_LIMITATIONS'
 assert (ROOT/'artifacts/handoffs/phase_03_to_phase_04/p03_handoff_manifest.json').is_file()
 assert (ROOT/'artifacts/handoffs/phase_03_to_phase_04/artifact_index.csv').is_file()

def test_p03_candidate_claim_and_selection_boundaries_are_preserved():
 d=json.loads((ROOT/'current/phase_03/CURRENT_PHASE_STATUS.json').read_text())
 assert d['candidate_claims_approved'] is False and d['claim_approval_performed'] is False
 assert d['test_used_for_selection'] is False
 assert d['threshold_reselection'] is False and d['threshold_retuning'] is False

def test_historical_p02_to_p03_handoff_and_dual_credential_contract_remain_preserved():
...
def test_prior_final_lifecycle_authorities_are_available():
 for p in [
