...
def audit_package(root: Path) -> dict[str, Any]:
    ...
    audit = root / 'reports/phase_00_closure_bridge/final_independent_audit_R3'
    ...
    handoff = load_yaml(root / 'reports/phase_00_closure_bridge/phase_0_layer0_evidence_map_layer10_handoff_R3.yaml')['phase_0_layer0_evidence_map_layer10_handoff']
    ...
    add('evidence_map_release_joins', all(r['execution_release_id'] == 'P00-EXECUTION-RELEASE-R3' and r['analysis_release_id'] == 'P00-ANALYSIS-RELEASE-R3' for r in rows), 'Evidence Map release join drift')
    add('source_inventory_current', {s['source_id'] for s in source_inventory['sources']} >= {'P00-EXECUTION-RELEASE-R3','P00-ANALYSIS-RELEASE-R3','P00-LAYER0-RELEASE-R3','P00-EVIDENCE-MAP-RELEASE-R3'}, 'Layer10 source inventory stale')
    ...
    add('layer10_sources_current', all({'P00-LAYER0-RELEASE-R3','P00-EVIDENCE-MAP-RELEASE-R3'}.issubset(set(v['source_releases'])) for v in view_catalog), 'Layer10 view source release drift')
    ...
    for entry in export_catalog:
        ...
        if obj.get('filter_id') != 'P00-FILTER-ALL-CURRENT-R3':
    ...
    add('report_noninterference', load_yaml(audit / 'report_noninterference_change_audit_R3.yaml')['analysis_release_changed'] is False, 'illegal analysis mutation')
    add('readjudication_complete', len(load_csv(audit / 'layer0_independent_readjudication_matrix_R3.csv')) == 7, 'readjudication matrix incomplete')
    add('evidence_map_audit_complete', len(load_csv(audit / 'evidence_map_closure_audit_R3.csv')) == 7, 'Evidence Map audit incomplete')
    add('layer10_audit_complete', len(load_csv(audit / 'layer10_source_value_read_only_audit_R3.csv')) == 14, 'Layer10 value audit incomplete')
    ...
    return {
        'validation_id': 'P00-L0-EMAP-L10-FINAL-AUDIT-VALIDATION-R3',
